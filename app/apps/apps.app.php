...
class appsApp {
    ...
    public function gets() {
        ...
        if(isset($_GET['AUTHID'])){
            ...
            $v OR iPHP::error_404('AUTHID 请求出错', 10001);
        }
        return array($v,$p,$f);
    }
...
//--------------------------------------------------------------------
    public static function render($data,$tpl,$name=null,$p=null) {
        ...
        if($data['category']){
            if(!isset(iView::$handle->_vars['APP'])){
                iView::assign('APP', $data['category']['app']); //当前应用信息
            }
    ...
    public static function process($expr,&$category,&$data){
        ...
        if(self::__is_html($expr,$category,$_app)){
            return false;
        }
    ...
    private static function __is_html($expr,$C,$key) {
        if (iView::$gateway == "html"
            && $expr
            && (
